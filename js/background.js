function main() {
	if(window.location.host.includes("twitter")) {
		const tweets = document.querySelectorAll('[data-testid="tweet"]')
		Array.from(tweets).forEach(readAndFlag)
	}
};

const CLASS_FLAG = 'hashtag_flagged';
...
const ValidHashtagList = [{
	hashtag: 'مهسا_امینی',
	regex: /([مهساامینی_ـ]+)/gm
}, {
	hashtag: 'mahsaamini', 
	regex: /([mahsaamini]+)/gm
}, {
	hashtag: 'اعتصابات_سراسری',
	regex: /([اعتصاباتسراسری_-]+)/gm
}, {
	hashtag: 'opiran',
	regex: /([opiran]+)/gm
}]
...
const hashtagRules = (text) => {
	return ValidHashtagList.some(({hashtag, regex}) => hashtagRuleCheck(text, regex, hashtag))
}

const hashtagRuleCheck = (text, regex, matcher) => {
	if(!!text.match(regex))
		return matcher === text
	// ignore
	return false;
